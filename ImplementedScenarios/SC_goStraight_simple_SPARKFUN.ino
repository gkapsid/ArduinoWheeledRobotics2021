// from https://gist.github.com/ShawnHymel/1de08ffaca990b65fade81cb8d01a44a
//gkapsid 25/10/2021

//tested
// seems to work decently!! maybe with some twiking would be perfect!!
...
const int counts_per_rev = 40;   // optical encoder 20 slots per rev tick on every change

// Pins
const int enc_l_pin = 0;          // Motor A
const int enc_r_pin = 1;          // Motor B
const int pwma_pin = 9;
const int ain1_pin = 7;
const int ain2_pin = 8;
const int pwmb_pin = 10;
const int bin1_pin = 11;
const int bin2_pin = 12;
const int stby_pin = 10; // NOT used to my setup with L298D bridge

// Globals
volatile unsigned long enc_l = 0;
volatile unsigned long enc_r = 0;

bool debug = false; //true in debug mode
bool leonardo = true; //false if UNO is used instead of LEONARDO
...
void driveStraight(float dist, int power) {
  ...
  Serial.print("Wheel circumference (mm)");
  Serial.println(wheel_c);
  Serial.print("number of rotations ");
  Serial.println(dist/(wheel_c/10));
  }
...
//************ NOT used with L293/298 H bridge *********************************
